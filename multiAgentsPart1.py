...
#PART 3.i
# The first algorithm created the largest number of nodes, this is because Regular Alpha-Beta pruning explores the successors of a node in a fixed order, 
# typically based on the order in which the actions are generated by the game rules. 
# This means that some branches of the game tree may be explored deeply before finding a cutoff, even if those branches are unlikely to lead to a good solution. 
# As a result, regular Alpha-Beta pruning may expand more nodes than necessary, leading to a larger search space.
# On the other hand, Best-First Alpha-Beta pruning uses a heuristic evaluation function to order the successors of a node based on their estimated potential for leading to a good solution. 
# This allows it to prioritize exploring the most promising nodes first, potentially finding cutoffs earlier and pruning more branches that are unlikely to lead to a good solution. As a result, Best-First Alpha-Beta pruning can often expand fewer nodes than regular Alpha-Beta pruning, leading to a smaller search space and potentially faster search times.

#PART 3.ii
# Regular Alpha-Beta pruning pruned more nodes than Best-First Alpha-Beta pruning, 
# as it  pruned entire branches of the game tree that are guaranteed to be worse than the current best solution without even exploring them. 
# In contrast, Best-First Alpha-Beta pruning prioritized exploring promising nodes first based on the heuristic evaluation function, 
# but it may still need to explore less promising nodes later in the search process if they have not been pruned earlier.

#PART3.iii
# Best-First Alpha-Beta pruning uses a heuristic evaluation function to guide the search towards more promising nodes first. However, the heuristic used in Best-First Alpha-Beta pruning did not significantly affect the search order
